...
function commandMenu:Init()
	...
	for categoryIndex, categoryName in ipairs(BSU.GetCommandCategories()) do
		local category = commandList:Add(categoryName)
		for commandIndex, command in ipairs(BSU.GetCommandsByCategory(categoryName)) do
...
hook.Add("BSU_BSUMenuInit", "BSU_CommandMenuInit", function(bsuMenu)
	local commandMenu = vgui.Create("BSUCommandMenu", bsuMenu)
	bsuMenu:AddTab("Commands", 1, commandMenu, "icon16/cog.png")
end)